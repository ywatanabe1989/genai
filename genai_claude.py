...
def run_genai(
    ...
    # Create conversation file with history
    conversation_data = _prepare_claude_conversation(
        ai_history[-n_history:], ai_prompt
    )
    ...
    _save_updated_ai_history_claude(
        ai_history, ai_history_path, ai_prompt, llm_out
    )
...
def _prepare_claude_conversation(history, current_prompt):
    messages = []
